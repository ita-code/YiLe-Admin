  ...
  <teleport to="body">
    <div class="hm-popup">
      ...
      <transition name="fade">
        <!-- 显示隐藏不能绑定到最外层，不然动画失效 -->
        <div class="w-screen h-screen bg-zinc-900/80 z-40 fixed top-0 left-0" @click="isOpen = false" v-if="isOpen"></div>
      </transition>
      <!-- 下方content  -->
      <transition name="popup-down-up">
        <div v-bind="$attrs" class="w-screen bg-white z-50 fixed bottom-0" v-if="isOpen">
          <slot></slot>
        </div>
      </transition>
    </div>
